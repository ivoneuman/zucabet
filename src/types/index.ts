...
// Pontuacao detalhada de um palpite
export interface ScoreBreakdown {
    result: number        // 3 se acertou vitoria/empate/derrota
  brazil_goals: number  // 1 se acertou gols do Brasil
  opp_goals: number     // 1 se acertou gols do adversario
  exact_bonus: number   // 5 se placar exato
  first_goal: number    // 1 se acertou primeiro gol
  var: number           // 1 se acertou VAR
  penalty: number       // 1 se acertou penalti
  header: number        // 1 se acertou cabecada
  yellow_cards: number  // 1 se acertou cartoes amarelos
  overtime: number      // 1 se acertou prorrogacao
  penalty_shootout: number // 1 se acertou disputa de penaltis
  total: number
}
